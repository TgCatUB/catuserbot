...
from userbot.utils import progress, command
...
@command(pattern="^.upload (.*)", allow_sudo=True)
async def _(event):
    if event.fwd_from:
        return
...
@command(pattern="^.uploadasstream (.*)", allow_sudo=True)
async def _(event):
    if event.fwd_from:
        return
    mone = await event.reply("Processing ...")
