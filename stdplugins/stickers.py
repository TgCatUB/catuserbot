...
@borg.on(admin_cmd(pattern="kangsticker ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    await event.edit("Pirating this sticker. Please Wait!")
    ...
    await event.edit(f"Yu ho!! Sticker Pirated! This sticker is [here](t.me/addstickers/{packshortname})")


@borg.on(admin_cmd(pattern="packinfo", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    if not event.is_reply:
...
@borg.on(admin_cmd(pattern="getsticker ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
