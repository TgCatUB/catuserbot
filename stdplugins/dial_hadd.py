...
@borg.on(admin_cmd(pattern="hadd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    packname = f"Nagato's Hindi Dialogue Pack 1.0"
    packshortname = f"Uniborg_Hindi"  # format: Uni_Borg_userid
    ...
    await event.edit("Adding this dialogue as sticker to hindi pack. Thand Rakh! Bot hu, Bhagwan Nahi xD")
    ...
    await event.edit(f"Yu ho!! Sticker Added To your Hindi Dialogue Pack! RIP This sticker's Soul Lies here [here](t.me/addstickers/{packshortname})")
