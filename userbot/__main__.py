...
def verifyLoggerGroup():
    ...
    if PM_LOGGER_GROUP_ID:
        try:
            entity = catub.loop.run_until_complete(catub.get_entity(PM_LOGGER_GROUP_ID))
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
        ...
        LOGS.info(
            "You haven't set the PM_LOGGER_GROUP_ID in vars please set it for proper functioning of userbot."
        )
