...
@borg.on(admin_cmd(pattern="dadd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    packname = f"Nagato's Dialogue Pack 2.0"
    packshortname = f"Uniborg_2"  # format: Uni_Borg_userid
    ...
    await event.edit("Adding this dialogue as sticker to pack. Please Wait!")
    ...
    await event.edit(f"Yu ho!! Sticker Added! This sticker is [here](t.me/addstickers/{packshortname})")
