...
async def edit_or_reply(event, text, parsemode=None,linkpreview=None):
    ...
    if parsemode:
        if event.sender_id in Config.SUDO_USERS:
            ...
            if reply_to:
                return await reply_to.reply(text, parse_mode=parsemode,link_preview=linkpreview)
            return await event.reply(text, parse_mode=parsemode,link_preview=linkpreview)
