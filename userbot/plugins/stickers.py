...
async def _(event):
    ...
    packname = f"@mrconfused  {userid}"
    packshortname = f"sandeep_cat{userid}"  # format: Uni_Borg_userid
    ...
    if is_a_s:
        ...
        packname = f"{userid}'s @mrconfused"
        if userid == 719877937:
            packshortname = "sandeep__cat"
        else:
            packshortname = f"sandeep_{userid}_as" # format: Uni_Borg_userid
