import os
from . import CMD_HELP , runcmd , make_gif
...
async def collage(cat):
    ...
    cat = await edit_or_reply(cat, "```collaging this may take several minutes too..... 😁```")
    ...
    if catinput:
        ...
        if not 0 < catinput < 10:
            os.remove(catsticker)
            await cat.edit("`Why too big grid you cant see images, use size of grid between 1 to 9`")
            return
    else:
        catinput = 3
    ...
    if not os.path.exists(endfile):
        for files in (catsticker,collagefile):
            if files and os.path.exists(files):
                os.remove(files)
        return await edit_delete(cat , f"`media is not supported or try with smaller grid size`" , 5)
