...
@catub.tgbot.cat_cmd(pattern="^/start({username})?([\s]+)?$",incoming=True, func=lambda e: e.is_private)
async def bot_start(event):
    ...
    try:
        ...
        if BOTLOG:
            await event.client.send_message(BOTLOG_CHATID,f"**Error**\nThere was a error while user starting your bot.\
                \n`{str(e)}`")
    else:
...
async def bot_pms(event):
    ...
    if chat.id != Config.OWNER_ID:
        ...
        try:
            ...
            LOGS.error(str(e))
            if BOTLOG:
                await event.client.tgbot.send_message(BOTLOG_CHATID, f"**Error**\nWhile storing messages details in database\n`{str(e)}`")
    else:
        reply_to = await reply_id(event)
        ...
        if user_id is not None:
            try:
                if event.media:
                    await event.client.tgbot.send_file(user_id,event.media,caption=event.text,reply_to=reply_msg)
                else:
                    await event.client.tgbot.send_message(user_id,event.text,reply_to=reply_msg)
            except Exception as e:
