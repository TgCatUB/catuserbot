...
class googleimagesdownload:
    ...
    def _get_image_objects(self,s):
        ...
        key = requests.post('https://nekobin.com/api/documents', json={"content": object_decode[:-14]}).json().get('result').get('key')
        url = f'https://nekobin.com/{key}'
        LOGS.info(url)
        image_objects = json.loads(object_decode[:-14])[31][0][12][2]
