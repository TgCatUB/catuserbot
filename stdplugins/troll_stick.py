...
@borg.on(admin_cmd(pattern="dede ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    packname = f"Nagato's Troll Pack @UniBorg"
    packshortname = f"Troll_Borg_{userid}"  # format: Uni_Borg_userid
    ...
    await event.edit("Yeh sticker mujhe de de. Thakur!")
    ...
    await event.edit(f"Gabbar khush hua! This sticker is [here](t.me/addstickers/{packshortname})")
